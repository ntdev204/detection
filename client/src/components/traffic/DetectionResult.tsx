...
export default function TrafficDetectionResult({ result, isLoading }: DetectionResultProps) {
  if (isLoading) {
    return (
      <Card>
  ...
  const vehicleEntries = Object.entries(result.class_counts).filter(([, v]) => v > 0);
      ...
      <Card>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold leading-tight">Phát hiện giao thông</p>
        ...
        {vehicleEntries.length > 0 && (
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {vehicleEntries.map(([cls, count]) => (
              <div key={cls} className="flex items-center justify-between rounded-lg bg-muted/40 px-3 py-2">
                <span className="text-xs text-muted-foreground">{VEHICLE_LABELS[cls] ?? cls}</span>
